    ...
    def del_port(self, ip, port):
        for token, target in self.storage.items():
            if target == f"{ip}:{port}":
                ...
                self.cfg_save()
                print(f"已删除 VNC: {ip}:{port}")
                return True
        print(f"未找到 VNC: {ip}:{port}")
        return False
...
class VProcess:
    ...
    def start(self):
        ...
        self.proc.start()
        print(f"websockify 服务已在新进程中启动 (PID: {self.proc.pid})")

    def close(self):
        """关闭 websockify 服务进程"""
        if self.proc is None:
            print("没有运行中的 websockify 服务")
            return

        if self.proc.is_alive():
            print(f"正在关闭 websockify 服务进程 (PID: {self.proc.pid})...")
            ...
            print("websockify 服务进程已关闭")
        else:
            print("websockify 服务进程已停止")
